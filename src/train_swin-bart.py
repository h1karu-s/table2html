...
from data.preprocessing_for_donut_large_truncation import preprocessing_data
from model.swin_bart import DonutConfig, DonutModel
...
class collate_fn:
    ...
    def __call__(self, batch):
        ...
        encoder_inputs = []
        for b in batch:
            image = Image.open(b["encoding_inputs"])
            encoder_encoding = self.encoder_processor(image).unsqueeze(0)
            encoder_inputs.append(encoder_encoding)
...
def main(args):
    ...
    if args.input_width is not None and args.input_height is not None:
        donut_config = DonutConfig(
            ...
            encoder_layer=args.encoder_depths,
    ...
    if args.model_path is not None:
        logger.info(f"Model: use {args.model_path}'s parameter")
        model = DonutModel(donut_config)
    else:
        logger.info(f"Model: use swin's parameter")
        model = DonutModel(donut_config)
        ...
        if (epoch+1) % args.save_freq == 0:
            if args.use_ddp == True:
                if dist.get_rank() == 0:
                    ...
                    # save_prediction(generated_html_train, html_train, f"{args.output}/epoch_{epoch}/predict_html_train.txt")
                    # save_prediction(generated_html_val, html_val, f"{args.output}/epoch_{epoch}/predict_html_val.txt")
            else:
                save_checkpoint(args, epoch, model.module, optimizer, lr_scheduler, scaler, logger, store)
...
def train_one_epoch(args, model, criterion, dataloader_train, dataloader_val, optimizer, epoch, lr_scheduler, scaler, device):
    ...
    for idx, (encoder_inputs, decoder_inputs, labels) in enumerate(dataloader_train):
        encoder_inputs = encoder_inputs.cuda(non_blocking=True)
...
def generation(args, data_loader, model):
    ...
    with torch.no_grad():
        for idx, (encoder_inputs, decoder_inputs, _) in enumerate(data_loader):
            generate_text = model.module.inference(image_tensors=encoder_inputs)["predictions"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--encoder_depths", nargs="*", type=int)
    parser.add_argument("--encoder_num_heads", nargs="*", type=int)
    ##other
    ...
    #fintabnet
    # add_vocab_list = ["<table>", "</table>", "<td>", "</td>", "<thead>", "</thead>", "<tr>", "<td", "</tr>", "rowspan=", "colspan=", "<sup>", "</sup>", "<b>", "</b>", "<i>", "</i>", "<sub>", "</sub>"]
