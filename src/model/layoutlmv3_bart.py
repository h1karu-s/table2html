"""
encoder: swin transformer(small)
decoder: mbeart
encoderとdecoderの次元が合わないため, cross_attenの前にliner層を追加
"""
"""
Donut
...
from transformers import MBartConfig, MBartForCausalLM, XLMRobertaTokenizer, AutoModel, AutoProcessor, LayoutLMv3Model, LayoutLMv3Config, LayoutLMv3Processor
...
class LayoutLMEncoder(nn.Module):
    def __init__(
        ...
        print(f"MODEL_ENCODER: init weight of LayoutLMv3_Encoder by [microsoft/layoutlmv3-base]" if not name_or_path else f"MODEL_ENCODER: init weight of LayoutLMv3_Encoder by {name_or_path}")
        state_dict = LayoutLMv3Model.from_pretrained("microsoft/layoutlmv3-base" if not name_or_path else name_or_path).state_dict()
        new_state_dict = self.model.state_dict()
        for x in new_state_dict:
            if x.endswith("embeddings.position_embeddings.weight") and new_state_dict[x].shape[0] != 514:
                new_state_dict[x] = state_dict[x][:new_state_dict[x].shape[0], :]
            elif x.endswith("embeddings.position_ids") and new_state_dict[x].shape[-1] != 514:
                new_state_dict[x] = state_dict[x][:, :new_state_dict[x].shape[-1]]
            else:
    ...
    def prepare_input(self, img: PIL.Image.Image, tokens: List[str], bboxes: List[List[int]]):
        return self.processor(img, tokens, boxes=bboxes, max_length=self.max_length, padding="max_length", truncation=True, return_tensors="pt")
...
class BARTDecoder(nn.Module):
    ...
    def __init__(
        self, decoder_layer: int, max_position_embeddings: int, name_or_path: Union[str, bytes, os.PathLike] = None, add_vocab_list: List[str] = None
    ):
        super().__init__()
        self.decoder_layer = decoder_layer
        self.max_position_embeddings = max_position_embeddings
        self.add_vocab_list = add_vocab_list
        ...
        if self.add_vocab_list is not None:
            print(f"MODEL_TOKENIZER: adding vocab is {self.add_vocab_list}")
            self.tokenizer.add_tokens(sorted(set(self.add_vocab_list)))
            self.tokenizer.clean_up_tokenization = self.clean_up_tokenization
        ...
        # weight init with asian-bart (change code)
        if not name_or_path:
            print("MODEL_DECODER: init weight of [facebook/mbart-large-50]")
            bart_state_dict = MBartForCausalLM.from_pretrained("facebook/mbart-large-50").state_dict()
            new_bart_state_dict = self.model.state_dict()
            for x in new_bart_state_dict:
                if x.endswith("embed_positions.weight") and self.max_position_embeddings != 1024:
                    print(x)
                    new_bart_state_dict[x] = self.resize_bart_abs_pos_emb(bart_state_dict[x], self.max_position_embeddings+2)
                elif x.endswith("embed_tokens.weight") or x.endswith("lm_head.weight"):
                    # For diff of vocab size, resize weight
                    if new_bart_state_dict[x].shape[0] > bart_state_dict[x].shape[0]:
    ...
    def clean_up_tokenization(self, out_string: str) -> str:
        ...
        for token in self.add_vocab_list:
            out_string = out_string.replace(f" {token} ", f"{token}").replace(f" {token}", token).replace(f"{token} ", token)
        out_string = (
            out_string.replace("colspan=", " colspan=")
    ...
    def prepare_inputs_for_inference(self, input_ids: torch.Tensor, encoder_outputs: torch.Tensor, past_key_values=None, past=None, use_cache: bool = None, attention_mask: torch.Tensor = None, encoder_attention_mask: torch.Tensor = None):
        """
        Args:
...
class DonutConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        encoder_name_or_path: Union[str, bytes, os.PathLike] = "",
        decoder_name_or_path: Union[str, bytes, os.PathLike] = "",
        add_vocab_list: List[str] = None,
        **kwargs,
    ):
        super().__init__()
        ...
        self.decoder_layer = decoder_layer
        self.max_position_embeddings = decoder_max_length if max_position_embeddings is None else max_position_embeddings
        self.encoder_max_length = encoder_max_length
        self.decoder_max_length = decoder_max_length
        self.encoder_name_or_path = encoder_name_or_path
        self.decoder_name_or_path = decoder_name_or_path
        self.add_voab_list = add_vocab_list
...
class DonutModel(PreTrainedModel):
    ...
    def __init__(self, config: DonutConfig):
        ...
        self.config = config
        self.encoder = LayoutLMEncoder(
            max_length=self.config.encoder_max_length,
            num_layers=self.config.encoder_layer,
            name_or_path = self.config.encoder_name_or_path
        )
        self.decoder = BARTDecoder(
            max_position_embeddings=self.config.max_position_embeddings,
            decoder_layer=self.config.decoder_layer,
            name_or_path=self.config.decoder_name_or_path,
            add_vocab_list=self.config.add_voab_list,
        )
        self.encoder_head = nn.Linear(768, self.decoder.model.config.d_model)

    def forward(self, inputs: Dict, decoder_input_ids: torch.Tensor, decoder_labels: torch.Tensor, attention_mask: Optional[torch.Tensor] = None,):
        """
        ...
        encoder_outputs = encoder_outputs[:, :self.config.encoder_max_length, :]
        encoder_outputs = self.encoder_head(encoder_outputs)
    ...
    def inference(
        ...
        encoder_outputs = self.encoder(inputs)
        encoder_outputs = encoder_outputs[:, :self.config.encoder_max_length, :]
        last_hidden_state = self.encoder_head(encoder_outputs)
