        ...
        if not name_or_path:
            ...
            for x in new_swin_state_dict:
                if x.endswith("relative_position_index") or x.endswith("attn_mask"):
                    pass
                elif not x in swin_state_dict.keys():
                    y = x.split(".")
                    num_block = int(y[3]) - 2
                    y[3] = str(num_block)
                    y = ".".join(y)
                    if x.endswith("relative_position_bias_table"):
                        new_swin_state_dict[x] = new_swin_state_dict[y]
                    else:
                        new_swin_state_dict[x] = swin_state_dict[y] 
                elif (
...
class BARTDecoder(nn.Module):
    ...
    def __init__(
        ...
        if not name_or_path:
            print("init weight of <facebook/mbart-large-50>")
...
class DonutConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.patch_size = patch_size
        self.embed_dim = embed_dim
        self.num_heads = num_heads if num_heads else [4, 8, 16, 32]
        self.decoder_layer = decoder_layer
...
class DonutModel(PreTrainedModel):
    ...
    def forward(self, image_tensors: torch.Tensor, decoder_input_ids: torch.Tensor, decoder_labels: torch.Tensor, attention_mask: Optional[torch.Tensor] = None,):
        """
    ...
    def inference(
        ...
        # last_hidden_state = self.encoder_head(last_hidden_state)
